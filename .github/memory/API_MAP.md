...
| `folly.ps1 <attune\|weave\|bind\|scry> [Debug\|Release]` (Windows) / `folly.sh <attune\|weave\|bind\|scry> [Debug\|Release]` (Linux/macOS) | This fork's wrapper around `eng/build.ps1`/`eng/build.sh` for `FollyOfTheUnbound.slnx`: `attune` restores, `weave` restores+builds, `bind` restores+builds+packs (via the real Arcade toolset, not a bare `dotnet build`/`pack`, which bypasses this repo's SDK bootstrap). `bind` also copies the produced nupkgs to `../.nupkg/FotU/<Configuration>`. `scry` restores+builds+tests, but the two wrappers differ here: `folly.sh scry` only runs CoreCLR tests (no `net472` runtime on Linux/macOS to run Desktop tests against), while `folly.ps1 scry` runs both a CoreCLR pass and a Desktop pass (moving the CoreCLR pass's `artifacts/TestResults`/`artifacts/log` to a `-CoreClr`-suffixed directory first so the Desktop pass's same-named result files don't overwrite them). |
