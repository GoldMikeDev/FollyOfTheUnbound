...
| `folly.ps1` / `folly.sh` | Active | This fork's own build/pack/test wrapper for `FollyOfTheUnbound.slnx` (`attune`/`weave`/`bind`/`scry`, mapping to Arcade's `--restore`/`--build`/`--pack`/test) — Windows and Linux/macOS respectively. Not fully in sync: `scry` runs both CoreCLR and Desktop tests on Windows (`folly.ps1`) but only CoreCLR on Linux/macOS (`folly.sh`, since there's no `net472` runtime there). See `API_MAP.md`'s Build & Test Entry Points table for the action mapping. |
