...
| `folly.ps1` / `folly.sh` | Active | This fork's own build/pack wrapper for `FollyOfTheUnbound.slnx` (`attune`/`weave`/`bind`, mapping to Arcade's `--restore`/`--build`/`--pack`) — Windows and Linux/macOS respectively, kept in sync. See `API_MAP.md`'s Build & Test Entry Points table for the action mapping. |
