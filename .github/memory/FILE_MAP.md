...
| `Tools/` | compiler | Compiler tooling (BuildBoss, format tools) and benchmark harnesses. Includes `RunTests/`, the custom parallel test runner behind `eng/build.ps1`/`eng/build.sh` and this fork's `folly.ps1`/`folly.sh scry`, with a live in-place progress table (frozen header, keyboard/mouse-wheel-scrollable body) on interactive terminals -- Windows mouse-wheel support uses CsWin32-generated console-mode bindings declared in `RunTests/NativeMethods.txt` (tested by the sibling `RunTests.UnitTests/`). |
...
| `folly.ps1` / `folly.sh` | Active | This fork's own build/pack/test wrapper for `FollyOfTheUnbound.slnx` (`attune`/`weave`/`bind`/`scry`, mapping to Arcade's `--restore`/`--build`/`--pack`/test) — Windows and Linux/macOS respectively. Not fully in sync: `scry` runs both CoreCLR and Desktop tests on Windows (`folly.ps1`, restrictable to just one via `--core`/`--desktop`) but only CoreCLR on Linux/macOS (`folly.sh`, since there's no `net472` runtime there, so no `--core`/`--desktop` there either). See `API_MAP.md`'s Build & Test Entry Points table for the action mapping. |
| `scripts/` | Active | Manually-invoked scripts not used by the build process (per its own `README.md`). Includes `test-folly-cleanse.sh` (standalone manual test harness for `folly.sh cleanse`), `test-folly-cleanse.ps1` (the same, for `folly.ps1 cleanse`'s bulk-delete path), and `test-folly-scry-args.ps1` (standalone regression harness for `folly.ps1 scry`'s argument parsing and unified test summary) — see `TESTING_STRATEGY.md`. |
