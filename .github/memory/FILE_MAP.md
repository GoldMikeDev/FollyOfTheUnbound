...
| `scripts/` | Active | Manually-invoked scripts not used by the build process (per its own `README.md`). Includes `test-folly-cleanse.sh` (standalone manual test harness for `folly.sh cleanse`) and `test-folly-scry-args.ps1` (standalone regression harness for `folly.ps1 scry`'s argument parsing and unified test summary) — see `TESTING_STRATEGY.md`. |
