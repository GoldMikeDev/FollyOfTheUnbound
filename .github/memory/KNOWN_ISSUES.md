...
- `FOTU_TEST_RESULTS_SUFFIX` gives `folly.ps1 scry`'s two legs (CoreCLR + Desktop) separate `TestResults`/log directories so they don't clobber each other. `eng/build.sh` has no Desktop/`.NET Framework` leg at all (`grep` for `test_desktop`/`--desktop`/`framework` in `build.sh` returns nothing) — Desktop-framework tests require Windows and cannot run on Linux/macOS. `folly.sh scry` only ever runs one leg (CoreCLR), so there is no directory collision to avoid and no purpose for the suffix mechanism.
