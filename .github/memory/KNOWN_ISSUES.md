...
`build-server shutdown` itself is not guaranteed to actually stop a server: it talks to the RPC pipe of servers registered by *the SDK it's invoked from*, so a build server started by a different dotnet install, or one whose RPC pipe is already wedged/orphaned, doesn't respond and survives the call silently while the CLI still reports success. Because of this, cleanse doesn't stop at diffing before/after PIDs to report a count for (1) — a PID gets force-killed directly (`Force-killed N build server process(es) that ignored 'dotnet build-server shutdown'.`) only once it satisfies all three: (a) it was already alive in the *original* pre-shutdown-call snapshot — a PID that only shows up in a later snapshot could be an unrelated process that started in between and must never be touched; (b) it's still alive after the shutdown call; and (c) its command line references this checkout's own bootstrapped `.dotnet` SDK root, the same scope (2)'s node-reuse workers are held to — a build server for a *different* repo's checkout or SDK matches the VBCSCompiler/Razor name pattern just as easily but must never be killed, since cleanse has no way to know whether it's mid-compile for someone else's work. Without the fallback at all, a build server left running by a foreign SDK or a stuck RPC pipe survives `cleanse` indefinitely with no error and no visible symptom beyond a `.NET host` process that never goes away; without the (a)-(c) scoping, the fallback itself becomes the hazard — collateral damage to an unrelated process. The kill itself also isn't a fire-and-forget signal: both `folly.sh`'s `_cleanse_kill_pid_tree` and `folly.ps1`'s `Stop-ProcessTree` send a graceful stop first (`SIGTERM` / `Stop-Process` without `-Force`), wait up to 5 seconds, and only escalate to a forceful kill (`SIGKILL` / `Stop-Process -Force`) if the process is still alive — a count is only incremented once the PID is confirmed gone by re-checking, since the signal being delivered (a non-error return from `kill`/`Stop-Process`) doesn't mean a process that traps or ignores the graceful signal actually died.
...
`folly.ps1`'s process matching (`Get-ProcessSnapshot`) uses `Get-CimInstance Win32_Process` on Windows and falls back to shelling out to `ps -eo pid,ppid,command` (the same source `folly.sh` uses) everywhere else. Before that fallback existed, `folly.ps1` called `Get-CimInstance` unconditionally — since that cmdlet is Windows-only WMI and PowerShell Core on Linux/macOS has no CIM server, it threw immediately and the *entire* cleanse action failed outright. This went unnoticed for a while because `folly.ps1` had never actually been run in a non-Windows sandbox; PRs touching it could only claim "written for parity, not runnable here". The `ps`-based fallback fixes that, but unlike `folly.sh` it has no self-exclusion for the invoking shell: if whatever launched `pwsh` has the search text (e.g. "VBCSCompiler") anywhere in *its own* command line — not `folly.ps1`'s own invocation, but an ancestor shell's — that ancestor matches and gets force-killed too. This isn't a concern for a normal terminal/CI invocation, but showed up hard when testing cleanse from Claude Code's own sandbox, where the harness wraps every Bash tool call as a single `bash -c '<entire command text>'`, so a test script that so much as defines the pattern string inline (e.g. in a heredoc) embeds it into that wrapper's own argv and makes the wrapper itself a match. When testing cleanse's build-server/node-worker killing logic in an agent sandbox, always launch the fake target process from a separate script file (`exec -a ... /path/to/script.sh`) rather than inline in the same tool call that contains the search pattern text, or the harness's own wrapper shell can get killed.
