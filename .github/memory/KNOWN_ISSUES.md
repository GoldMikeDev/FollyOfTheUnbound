...
`build-server shutdown` itself is not guaranteed to actually stop a server: it talks to the RPC pipe of servers registered by *the SDK it's invoked from*, so a build server started by a different dotnet install, or one whose RPC pipe is already wedged/orphaned, doesn't respond and survives the call silently while the CLI still reports success. Because of this, cleanse doesn't stop at diffing before/after PIDs to report a count for (1) — anything still alive from the *original* snapshot after the shutdown call is unconditionally stale (same reasoning as (2): cleanse never launches a build itself) and gets force-killed directly (`Force-killed N build server process(es) that ignored 'dotnet build-server shutdown'.`), the same way (2)'s node-reuse workers always were. Without this fallback, a build server left running by a foreign SDK or a stuck RPC pipe survives `cleanse` indefinitely with no error and no visible symptom beyond a `.NET host` process that never goes away.
