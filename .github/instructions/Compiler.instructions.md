...
- `src/Tools/` - Compiler tooling (BuildBoss, format tools, analyzers) and benchmark harnesses; `IdeCoreBenchmarks/BenchmarkDotNet.Directory.Packages.props` supplies the package policy copied beside generated BDN runners. `src/Tools/RunTests/` is the custom test runner behind `eng/build.ps1`/`eng/build.sh`'s `-test*`/`--test*` flags (and this fork's `folly.ps1`/`folly.sh scry`): partitions test assemblies into work items, runs them in parallel via `vstest.console.dll`, and detects test-host crashes/hangs from vstest's `/Blame` dump output (`ProcessTestExecutor.CheckForCrashes`, scoped to dumps created since that specific work item started -- the results directory and its dumps are shared and never cleaned up across work items/runs, so an unscoped scan would misattribute stale or concurrent dumps to unrelated failures). `TestRunner`'s run loop drives `LiveTestProgressDisplay`, an in-place redrawn table of running/failed/timed-out work items shown only on a real, non-redirected interactive terminal (`Console.IsOutputRedirected`); `eng/build.ps1`'s invocation of `RunTests.exe` must use `-useConsole` for that to ever be true outside CI, since the default piped invocation makes `Console.IsOutputRedirected` true inside `RunTests` regardless of whether the calling session itself is interactive.
