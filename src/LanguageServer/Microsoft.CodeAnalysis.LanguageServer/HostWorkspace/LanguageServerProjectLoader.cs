...
internal abstract partial class LanguageServerProjectLoader : IAsyncDisposable
{
    ...
    private async ValueTask ReloadProjectsAsync(AsyncPriorityWorkQueue<string>.WorkToProcess projectsToLoadOrReload, CancellationToken cancellationToken)
    {
        ...
        if (GlobalOptionService.GetConnectionScopedOption(LanguageServerProjectSystemOptionsStorage.EnableAutomaticRestore) && !projectsThatNeedRestore.IsEmpty)
        {
    ...
    public virtual async ValueTask DisposeAsync()
    {
        using (await _gate.DisposableWaitAsync(CancellationToken.None))
        {
            ...
            foreach (var (_, project) in _loadedProjects)
            {
                ...
                {
                    // LoadedProject.DisposeAsync() releases its file watches first and only then calls
                    ...
                    // DisposeAsync() (both are independent ILspServices torn down together), so this must
