...
/// <summary>
/// A subclass of this type (<c>CohostCompletionListCache</c>) is resolved from a <c>[Shared]</c>-composition
/// MEF part shared across every daemon connection (see GoldMikeDev/roslyn#9). The cache used to be one shared
/// circular buffer, so completion result ids from one connection could collide with, or be resolved against,
/// another connection's in-flight completion list. Each connection now gets its own <see cref="Impl"/> instance,
/// keyed by <see cref="AmbientConnectionToken.Current"/>, same pattern as <c>ClientSettingsManager</c>.
/// </summary>
internal class CompletionListCache
{
    ...
    private readonly ConditionalWeakTable<object, Impl> _implByConnection = new();
    private readonly Impl _implWithNoAmbientConnection = new();

    private Impl GetImpl()
        => AmbientConnectionToken.Current is { } token
            ? _implByConnection.GetOrCreateValue(token)
            : _implWithNoAmbientConnection;

    public int Add(VSInternalCompletionList completionList, ICompletionResolveContext context)
        => GetImpl().Add(completionList, context);
    ...
    public bool TryGetOriginalRequestData(VSInternalCompletionItem completionItem, [NotNullWhen(true)] out VSInternalCompletionList? completionList, [NotNullWhen(true)] out ICompletionResolveContext? context)
    {
        ...
        {
            if (impl.TryGet(resultId, out completionList, out context) &&
